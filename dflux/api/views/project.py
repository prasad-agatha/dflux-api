    ...
    * This endpoint will allows only GET, POST methods.
    ...
    def get(self, request):
        ...
        queryset = ProjectFilter(
            ...
            .filter(Q(user=request.user) | Q(projectmembers__user=request.user))
            .distinct(),
        ).qs
        # print(queryset)
...
class ProjectDetailView(BaseAPIView):
    ...
    * This endpoint will allows only GET, PUT, DELETE methods.
    ...
    def get_object(self, request, pk):
        ...
        try:
            project = Project.objects.select_related("user").get(id=pk)
            return project
...
class ProjectMemberDetailView(BaseAPIView):
    ...
    * This endpoint will allows only GET, PUT, DELETE methods.
    ...
    def get_object(self, pk):
        ...
        """
        try:
            return ProjectMembers.objects.get(pk=pk)
    ...
    def get(self, request, pk):
        ...
        """
        project_members = self.get_object(pk)
...
class UserProjectRoleView(BaseAPIView):
    ...
    * This endpoint will allows only GET method.
    ...
    def get(self, request, pk):
        members = ProjectMembers.objects.select_related("user", "project").filter(
            project__id=pk, user=request.user.id
        )
        serializer = ProjectMemberSerializer(members, many=True)
        return Response(
            {
                "user_email": serializer.data[0]["user"]["email"],
                "user_role": serializer.data[0]["role"],
                "user_access": serializer.data[0]["access"],
                "user_module_access": serializer.data[0]["module_access"],
            },
